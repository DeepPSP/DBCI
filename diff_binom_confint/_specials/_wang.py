...
def wang_binomial_ci(
    ...
    if sides == ConfidenceIntervalSides.TwoSided.value:
        ...
        ci_lower = binomial_ci_one(n_positive, n_total, ref_positive, ref_total, adjusted_conf_level, ConfidenceIntervalSides.LeftSided.value, precision, grid_one, grid_two)
        ci_upper = binomial_ci_one(n_positive, n_total, ref_positive, ref_total, adjusted_conf_level, ConfidenceIntervalSides.RightSided.value, precision, grid_one, grid_two)
...
def binomial_ci_one(n_positive, n_total, ref_positive, ref_total, conf_level, sides, precision, grid_one, grid_two) -> List[float]:
...
def _prob(delv, delta, n, m, i1, i2):
    ...
    part1 = (
        np.log(comb(n, i1))[:, np.newaxis] + 
        np.outer(i1, np.log(p0 + delv)) + 
        np.outer(n - i1, np.log(1 - p0 - delv))
    )
    
    part2 = (
        np.log(comb(m, i2))[:, np.newaxis] + 
        np.outer(i2, np.log(p0)) + 
        np.outer(m - i2, np.log(1 - p0))
    )
